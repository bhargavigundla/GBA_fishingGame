...
u16 hookMap[5][5] = {
 {0, 0, 0x52FF, 0, 0},
 {0x52FF, 0, 0x52FF, 0, 0x52FF},
 {0x52FF, 0, 0x52FF, 0, 0x52FF},
 {0x52FF, 0, 0x52FF, 0, 0x52FF},
 {0x52FF, 0x52FF, 0x52FF, 0x52FF, 0x52FF}
};
...
void updateHook() {
  ...
  if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<5)))) {
   ...
   hook.col += (hook.col + hook.width - 1 < 240 - hook.cdel) ? hook.cdel : 0;
  ...
  hook.row += (hook.row + hook.height - 1 < 160 - hook.rdel) ? hook.rdel : 0;
 }
}


void drawHook() {
  drawRect(hook.oldCol, hook.oldRow, hook.width, hook.height, 0x7352);
    int hCol = 0;
    int hRow = 0;
...
void initFishies() {
  for (int i = 0; i < 8; i++) {
   ...
   fishies[i].cdel = 1;
   fishies[i].color = ((rand() % 30) | (rand() % 31)<<5 | (rand() % 31)<<10);
   fishies[i].active = 1;
   fishies[i].erased = 0;
...
int updateFish(FISHIE* b) {
 ...
 if (b->active) {
   ...
   *(volatile u16*)0x04000068 = (((15) & 15) << 12) | (0 << 11) | (((7) & 7) << 8) | (2 << 6);
      *(volatile u16*)0x0400006C = NOTE_A7 | (1<<15) | (1<<14);
   if (b->evil) {
    return 1;
   } else {
    b->color = ((rand() % 30) | (rand() % 31)<<5 | (rand() % 31)<<10);
    score += 1;
...
void drawFish(FISHIE* b) {
 if (wait == maxWait) {
   drawRect(b->oldCol, b->oldRow, b->width, b->height, 0x7352);
